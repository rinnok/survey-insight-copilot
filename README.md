...
## 2つの分析経路（公開版で使えるのは汎用のほう）

| 経路 | 入口 | 使えるデータ | 公開版 |
|---|---|---|---|
| **汎用分析** | ホームの「どのアンケートでも分析する（汎用）」 | 任意のアンケートCSV/Excel | ✅ **これが公開デモの主導線** |
| セッション分析（熱海DR3） | ホームの「何を明らかにしたいか相談する」 | 熱海DMOの実データ**専用** | ⚠️ 実データが要る |

セッション分析（`py/atami_dr3.py`）は熱海DMOの調査票1本に固定された分析で、
特定の英語列名（`Are you currently a bachelors or masters student` など）と
実データの件数を前提にしています。**他のデータでは動きません。**
手元のアンケートを試す場合は必ず汎用分析を使ってください。

対話を止めた代わりに、セッション分析には「問いを直接入力して分析する」フォームを
足してあります（`confirm-question` → `analyze`。どちらもLLM不要）。
